/*************  ✨ Codeium Command 🌟  *************/
// The problem with this code is that the `JoditEditor` component is not being properly registered in the DOM. This is because the `ref` prop is not being passed to the component correctly. To fix this, the `ref` prop should be passed as a prop to the `JoditEditor` component like so: `<JoditEditor ref={(ref) => { editor.current = ref; }} ... />`. This will ensure that the component is properly registered in the DOM and that the `editor.current` property is set correctly.
...
// Dynamically load JoditEditor to handle SSR issues
...
const CreatePost = () => {
  ...
  const config: IJoditEditorProps["config"] = useMemo(
    () => ({
      ...
      style: {
        ".jodit-wysiwyg": {
          fontWeight: "normal", // Default font weight, can be customized
        },
      },
      buttons: [
        "bold",
  ...
  return (
    <>
      ...
      <div className="flex xl:flex-row flex-col gap-12 mt-12">
